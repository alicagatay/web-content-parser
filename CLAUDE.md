...
- **`google_drive.py`** — `create_google_doc()` (tags docs with `appProperties` for cross-mode dedup), `_collect_subtree_folder_ids_sync()` (BFS enumeration of the Resources folder + all nested subfolders), `_build_doc_title_cache_sync()`, `find_standalone_docs_for_urls_sync()`, `find_all_tabbed_base_urls_sync()`, and `sanitize_doc_title()`. Both orchestrators call `_collect_subtree_folder_ids_sync()` once and pass the resulting folder-ID list to the relevant checks, so each run walks the subtree a single time: `main()` shares it between `_build_doc_title_cache_sync()` and `find_all_tabbed_base_urls_sync()`; `main_recursive()` shares it between the tabbed-doc pre-check (`find_all_tabbed_base_urls_sync()`) and the Phase 4b cleanup (`find_standalone_docs_for_urls_sync()`). Google API services are created once and passed as parameters.
...
- **Doc title cache**: Built once per run via `_build_doc_title_cache_sync()`, covering the Resources folder **and every nested subfolder** (one doc-listing query per folder). This ensures a URL whose doc was filed into any subfolder is still detected as a duplicate, no matter where in the Resources tree it lives. `main()` enumerates the subtree once (`_collect_subtree_folder_ids_sync()`) and shares that folder list with the tabbed-doc scan, so the folder tree is walked a single time per normal-mode run.
...
- **Cross-mode duplicate detection**: Both modes tag Google Drive files with `appProperties` metadata (`doc_mode`, `source_url`/`base_url`). Normal mode skips URLs already covered by a recursive doc's `base_url` prefix. Recursive mode, before discovery, skips the whole crawl if an existing tabbed doc's `base_url` already covers the requested URL (`is_within_prefix`), and after creation deletes standalone docs whose `source_url` matches a sub-page. Zero extra API calls for tagging (merged into existing `files().update`). All cross-mode lookups (`find_all_tabbed_base_urls_sync()`, `find_standalone_docs_for_urls_sync()`) also traverse the full Resources subtree, so docs filed in subfolders are matched too.
